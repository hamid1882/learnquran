...
const EChapters = () => {
  const [loading, data] = useRemoteChapters('https://api.quran.com/api/v4/chapters?language=en', (data) => data.chapters)
      ...
      {data.map((item) => {
        return (
          <div className="col col-md-4 col-lg-3  border border-success rounded text-center my-3" style={{cursor: 'pointer'}}>
            <Link to={`/echapters/${item.id}`} style={{ textDecoration: "none", color: "black" }} className="d-flex p-2  rounded justify-content-evenly align-items-center" key={item.id}>
